...
저장 경로: `D:\02.Dev_Project\webscrap\analysis\{stable_id}\result.html`

위 "잘림 방지 — 단계별 저장 절차"를 반드시 따른다.

**`<title>` 및 `.report-header h1` 형식 (필수)**:
- `<title>` 태그: `[{stable_id}] {공고명}`
- `.report-header h1`: `{공고명}` (사업명)
...
```
저장 완료: D:\02.Dev_Project\webscrap\analysis\{stable_id}\result.html
```
